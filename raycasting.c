...
int	raycasting(t_big *big)
{
	...
	big->ray.posX = 3, big->ray.posY = 3;  //x and y start position
	...
	while(big->ray.x < w)
	{
		...
		big->ray.mapX = (int)big->ray.posX;
      	big->ray.mapY = (int)big->ray.posY;
		...
		{
			big->ray.stepX = -1;
			big->ray.sideDistX = (big->ray.posX - big->ray.mapX) * big->ray.deltaDistX;
		}
		else
		{
			big->ray.stepX = 1;
			big->ray.sideDistX = (big->ray.mapX + 1.0 - big->ray.posX) * big->ray.deltaDistX;
		}
		if(big->ray.rayDirY < 0)
		{
			big->ray.stepY = -1;
			big->ray.sideDistY = (big->ray.posY - big->ray.mapY) * big->ray.deltaDistY;
		}
		else
		{
			big->ray.stepY = 1;
			big->ray.sideDistY = (big->ray.mapY + 1.0 - big->ray.posY) * big->ray.deltaDistY;
		}
		big->ray.hit = 0;
		while (big->ray.hit == 0)
		{
			//jump to next map square, OR in x-direction, OR in y-direction
			if(big->ray.sideDistX < big->ray.sideDistY)
			{
				big->ray.sideDistX += big->ray.deltaDistX;
				big->ray.mapX += big->ray.stepX;
				big->ray.side = 0;
			}
			else
			{
				big->ray.sideDistY += big->ray.deltaDistY;
				big->ray.mapY += big->ray.stepY;
				big->ray.side = 1;
			}
			//Check if ray has hit a wall
			if(big->map.mapa[big->ray.mapX][big->ray.mapY] != '0')
				big->ray.hit = 1;
		}
	  	//Calculate distance projected on camera direction (Euclidean distance will give fisheye effect!)
		if(big->ray.side == 0)
			big->ray.perpWallDist = (big->ray.mapX - big->ray.posX + (1 - big->ray.stepX) / 2) / big->ray.rayDirX;
		else
			big->ray.perpWallDist = (big->ray.mapY - big->ray.posY + (1 - big->ray.stepY) / 2) / big->ray.rayDirY;
